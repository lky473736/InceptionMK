...
class RotationDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if rotation_label == 1:
            data = torch.roll(data, shifts=data.shape[0]//4, dims=0)
        elif rotation_label == 2:
            data = torch.roll(data, shifts=data.shape[0]//2, dims=0)
        elif rotation_label == 3:
            data = torch.roll(data, shifts=3*data.shape[0]//4, dims=0)
        
        return data, rotation_label

def pretrain(backbone, train_loader, args):
    classifier = nn.Linear(args.feature_dim, 4).to(args.device)
