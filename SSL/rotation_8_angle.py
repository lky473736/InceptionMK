...
def apply_3d_rotation(data, angles):
    ...
    rm = torch.from_numpy(r).float().to(data.device)
    rotated_data = data.clone()
    for i in [0, 3, 6]:
        rotated_data[:, i:i+3, :] = torch.matmul(rm, data[:, i:i+3, :])
    return rotated_data

class Rotation8AngleDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        rotated_data = apply_3d_rotation(data.unsqueeze(0), [0, 0, self.angles[label]]).squeeze(0)
        return rotated_data, label

def pretrain(backbone, pretrain_loader, args):
    pretext_classifier = nn.Linear(args.feature_dim, 8).to(args.device)
...
if __name__ == '__main__':
    ...
    # pretrain_loader = DataLoader(Rotation8AngleDataset(base_train_dataset), batch_size=args.batch_size, shuffle=True)
